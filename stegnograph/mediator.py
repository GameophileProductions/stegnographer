...
# box image path - will be accepted as input while encrypting
box_image_path = "Yuji.jpeg"


# container image path - will be accepted as input while encrypting
container_image_path = "Sukuna.jpeg"

# used for decryption  #Will be accepted as input while decryption
encrypted_image_path = "encoded.tiff"


# the path of the encrypted file - used while decrypting
output_decrypted_file = "Decoded.tiff"  # this one will remain constant

# similar to encrypted image path but used for encryption output
output_encryped_file = "encoded.tiff"  # this one will remain constant
...
'''
 ...
 This saves the image file into the variable encrypted_image using the path of the encrypted image stored inside encrypted_image_path
encrypted_image = PIL.Image.open(mode='r', fp=encrypted_image_path)

 this will save the output of the decryption as Decoded.tiff - this will be the output file we will have to provide to the user
decode(encrypted_image, 2).save(output_decrypted_file)


               Example of Encryption
box_image = PIL.Image.open(mode='r', fp=box_image_path)
container_image = PIL.Image.open(mode='r', fp=container_image_path)
encode(box_image, container_image, 2).save(output_encryped_file)
'''
